...
class MockNetworkManager: NetworkManager {
  ...
  override func fetchSongs(
    completion: @escaping (Result<[Song], NetworkError>) -> Void
  ) {
...
class TestableMusicPlayerViewModel: MusicPlayerViewModel {
  ...
  private func setErrorMessage(for error: NetworkError) {
    ...
    case .decodingError(let decodingError):
      errorMessage =
        "Error decoding data: \(decodingError.localizedDescription)"
    case .noData:
...
struct NetworkTest {
  @Test(
    "Given initial state, When fetchSongs succeeds, Then songs are loaded and first song is selected"
  )
  ...
  @Test(
    "Given initial state, When fetchSongs is loading, Then isLoading is true"
  )
  ...
  @Test(
    "Given initial state, When fetchSongs fails with network error, Then error message is set"
  )
...
struct SearchTest {
  @Test(
    "Given songs loaded, When searching by track name, Then matching songs are filtered"
  )
  ...
  @Test(
    "Given songs loaded, When searching by artist name, Then matching songs are filtered"
  )
  ...
  @Test(
    "Given songs loaded, When searching with no matches, Then filtered songs is empty"
  )
...
struct PlayerFunctionalityTest {
  @Test(
    "Given song is not playing, When playPause is called, Then song starts playing"
  )
  ...
  @Test(
    "Given songs loaded, When selecting a different song, Then selected song plays"
  )
